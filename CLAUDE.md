...
uv run python manage.py fetch_crypto_prices # Fetch crypto prices + USD/PKR rates from CoinGecko
                                           # Options: --ticker BTC --start-date 2025-06-01 --end-date 2026-01-01 --no-exchange-rate
...
- **Investments:** Full portfolio tracking with Instrument, InstrumentPrice, InvestmentTransaction, ExchangeRate models. `Instrument.api_id` stores CoinGecko coin ID for auto-fetching crypto prices. Four transaction types: buy (deducts from account), sell (credits account), reinvestment (adds units/cost basis, zero balance impact), dividend (credits cash payout to account, no unit impact). Portfolio dashboard with Chart.js (allocation donut, 30-day performance line). Holdings table grouped by instrument type with unrealized/realized/dividends/total G/L columns. Bulk price entry for daily manual pricing. Excel price import (`PriceImportView`) with auto-detection of Meezan/MUFAP and MCB xlsx formats — upload → preview → confirm flow using `openpyxl`. Auto-fetch crypto prices from CoinGecko via `crypto_prices.py` — CLI command or UI button on bulk prices page with background threading and status polling. Last-known-price/rate logic for historical lookups. USD positions converted to PKR via ExchangeRate for portfolio totals. Uses purple accent color.
